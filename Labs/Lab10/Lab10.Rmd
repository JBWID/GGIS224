---
title: 'Jonathan, Environmental Data Science @UIUC. Lab 10 - Emission point analysis'
date: "4/5/2024"
output:
...
```{r GRADUATED SYMBOLOGY MAP WITH CHOROPLETH MAP OVERLAY}
...
```{r Re-Transform for Spatial Operations}
st_crs(emissions)
st_crs(counties)
...
```{r CALCULATE FACILITY COUNT BY COUNTY}
...
Answer: the unit is square meters because the st_area() returns area values in square meters by default even though hte CRS used has units in US survey feet, the st_area() function still returns the area in square meters.
...
```{r CALCULATE FACILITY EMISSION AVERAGE BY COUNTY}
...
```{r BUFFER ANALYSIS: considering that emission often diffuses from the point source.}

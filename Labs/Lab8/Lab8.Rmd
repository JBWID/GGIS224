...
zcta_data <- get_acs(geography = 'zcta', 
                     variables = c("B01001A_001", "B01001A_008", "B01001A_023"),
                     year = 2020, 
                     geometry = TRUE) %>%
  pivot_wider(
    names_from = variable, 
  ...
  mutate(
    percentage_20_24 = (`B01001A_008` + `B01001A_023`) / `B01001A_001` * 100
  ) %>%
  select(GEOID, NAME, `B01001A_001`, `B01001A_008`, `B01001A_023`, percentage_20_24, geometry)



# Step 6: Read Chicago boundary data and transform CRS
...
# Transform the CRS of zcta_data_wide to match chicago_boundary_zip
zcta_data_wide <- st_transform(zcta_data, 4326)

# Perform the intersection
intersected_data <- st_intersection(zcta_data_wide, chicago_boundary_zip)

# Ensure there are no empty geometries after the intersection
intersected_data <- intersected_data[!st_is_empty(intersected_data), ]

# Mapping the percentage of 20-24 year olds in each Chicago ZCTA
tm_shape(intersected_data) +
  tm_polygons("percentage_20_24", palette="-Blues", title="Percentage of 20-24 Year Olds") +
